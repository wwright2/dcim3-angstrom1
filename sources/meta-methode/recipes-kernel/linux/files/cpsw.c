...
static int cpsw_common_res_usage_state(struct cpsw_priv *priv)
{
	u32 i;
	u32 usage_count = 0;
	//printk("The no of slaves found: %d\n", priv->data.slaves);
	for (i = 0; i < priv->data.slaves; i++)
	{	
		//printk("Inside cpsw_common_res_usage_state 1 \n");
		if (priv->slaves[i].open_stat)
			{
			
				//printk("Inside cpsw_common_res_usage_state 2 \n");
				usage_count++;
			}
	}
	return usage_count;
...
static void cpsw_set_phy_config(struct cpsw_priv *priv, struct phy_device *phy)
{
	...
	/* In CPU Attached Mode (SW_MODE[1:0] = 00) 
	 * programmed 'Port Control Register' @ offset 0x4 [1:0] = '11' 
	 * to configure port in 'Forwarding State'.	*/
        miibus->write(miibus, 0x18, 0x04, 0x03);
	miibus->write(miibus, 0x19, 0x04, 0x03);
        miibus->write(miibus, 0x1a, 0x04, 0x03);
        miibus->write(miibus, 0x1b, 0x04, 0x03);
	miibus->write(miibus, 0x1c, 0x04, 0x03);
	miibus->write(miibus, 0x1d, 0x04, 0x03);
...
static int cpsw_ndo_open(struct net_device *ndev)
{
	...
	if (!cpsw_common_res_usage_state(priv)) {
		...
		if (ret < 0) {
			...
			printk("Error adding device attr  code returned \n");
			return ret;
		}
...
static int __devinit cpsw_probe(struct platform_device *pdev)
{
	...
	if (!priv->slaves) {
		...
		ret = -EBUSY;
		printk(" Failed to allocate the slave ports \n");
		goto clean_ndev_ret;
	}
	...
	if (!priv->cpsw_res) {
		...
		printk(" Failed to get i/o resources\n");
		ret = -ENOENT;
		goto clean_clk_ret;
	}
	...
	if (!request_mem_region(priv->cpsw_res->start,
		...
		printk(" Failed to get mem_region \n");
		ret = -ENXIO;
		goto clean_clk_ret;
	}
	...
	if (!priv->cpsw_ss_res) {
		dev_err(priv->dev, "error getting i/o resource\n");
		printk(" Failed to get platform resource \n");
		ret = -ENOENT;
		goto clean_clk_ret;
	}
	...
	if (!request_mem_region(priv->cpsw_ss_res->start,
		...
		dev_err(priv->dev, "failed request i/o region\n");
		printk(" Failed to get mem region \n");
		ret = -ENXIO;
		goto clean_clk_ret;
	}
	...
	if (!dmtimer_rx) {
		...
		printk(" Failed to get TX timer request \n");
		ret = -ENODEV;
	...
	if (!dmtimer_tx) {
		...
		printk(" Failed to get RX timer request \n");
		ret = -ENODEV;
	...
	if (WARN_ON(!priv->txch || !priv->rxch)) {
		...
		ret = -ENOMEM;
		printk(" Failed to initialize dma channels 2  \n");
		goto clean_dma_ret;
	}
	...
	if (!priv->ale) {
		...
		ret = -ENODEV;
		printk(" Failed to initialize ale engine \n");
		goto clean_dma_ret;
	}
...
//		printk(" Failed to allocate the slave ports \n");
	if (ret) {
		printk(" Failed to init slave emac \n");
